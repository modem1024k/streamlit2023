...
def calculate_equal(amount, annual_interest_rate, loan_term,sel_day):
    ...
    # 计算每期还款金额
    monthly_payment = amount * (monthly_interest_rate * (1 + monthly_interest_rate) ** loan_term) / ((1 + monthly_interest_rate) ** loan_term - 1)
...
def hotel():
    ...
    with col1:
        if st.button("开始测算"):
            if operation == "计算":
                result,payment = cal_hotel(num1,num2,num3,num4,rate_count,selected_date)
                st.write(result)
...
#统计报表展示
def html_report():
    rq = st.text_input('查询日期  贷款情况(万元)',"202405")
...
#统计报表展示
def ph_report():
    rq = st.text_input('查询日期  普惠情况(万元)',"202405")
